...
{
    [CustomEditor(typeof(NATLobbyManager), true)]
    [CanEditMultipleObjects]
    class NATLobbyManagerEditor : NATNetworkManagerEditor
    {
        ...
        void InitLobby()
        {
            if (!m_Initialized)
            {
                ...
                var lobby = target as NATLobbyManager;
                if (lobby == null)
                    return;
        ...
        virtual public void ShowNATTraversalProperties()
        {
            bool first = true;
            ...
            {
                var f = typeof(NATTraversal.NetworkManager).GetField(property.name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                var p = typeof(NATTraversal.NetworkManager).GetProperty(property.name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);

                if (f == null && p == null) continue;

                f = typeof(UnityEngine.Networking.NetworkManager).GetField(property.name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                p = typeof(UnityEngine.Networking.NetworkManager).GetProperty(property.name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
        ...
        void SetLobbyScene(NATLobbyManager lobby, string sceneName)
        {
        ...
        void SetPlayScene(NATLobbyManager lobby, string sceneName)
        {
...
namespace NATTraversal
{
    ...
    class NATLobbyManagerEditor : NATNetworkManagerEditor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(m_MinPlayersProperty, m_MinPlayersLabel);
            EditorGUILayout.PropertyField(m_LobbyPlayerPrefabProperty);
            EditorGUILayout.PropertyField(m_GamePlayerPrefabProperty);
        ...
        protected void ShowLobbyScenes()
        {
            ...
            var offlineObj = GetSceneObject(lobby.lobbyScene);
            var newOfflineScene = EditorGUILayout.ObjectField(m_LobbySceneLabel, offlineObj, typeof(UnityObject), true);
            if (newOfflineScene == null)
            {
            ...
            else
            {
                if (newOfflineScene.name != m_NetworkManager.offlineScene)
                {
                    ...
                    lobby.lobbyScene = newOfflineScene.name;
                    EditorUtility.SetDirty(target);
                }
            }

            var onlineObj = GetSceneObject(lobby.playScene);
            var newOnlineScene = EditorGUILayout.ObjectField(m_PlaySceneLabel, onlineObj, typeof(UnityObject), true);
            if (newOnlineScene == null)
            {
            ...
            else
            {
                ...
                {
                    m_NetworkManager.onlineScene = "";
                    lobby.playScene = newOnlineScene.name;
                    EditorUtility.SetDirty(target);
                }
            }
        }
